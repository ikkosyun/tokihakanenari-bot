...
- **`_generate()`内でリール生成の例外を握りつぶさないと、フィード/ストーリーの
  生成まで巻き添えで失敗する。** 2026-07-22、`ubuntu-latest`にffmpegが標準搭載
  という前提が誤りで(`shutil.which("ffmpeg")`が失敗)、これがリール生成だけでなく
  日次投稿全体を失敗させた。対策: ワークフローで`apt-get install -y ffmpeg`を
  明示的に実行する(コミット`6a46445`)のに加え、`main.py._generate()`内の
  リール生成をtry/exceptで囲み、リール側の失敗が本体(フィード/ストーリー)の
  生成・コミットを止めないようにした。

### インシデント 2026-07-22/23: 「API access blocked」でpublishが終日失敗

`instagram_post.post_image()`等が`{"error":{"message":"API access blocked.",
"type":"OAuthException","code":200}}`で失敗し、約9時間(投稿だけでなく
`GET /{ig_user_id}?fields=username`のような読み取り専用リクエストまで)
ブロックされ続けた。Meta開発者ダッシュボードには特に警告表示はなし。

**真因: レート制限ではなく、Instagramアカウント側の本人確認ロックだった。**
Instagramアプリを開いたところ本人確認(アカウント認証)を求められ、それを
完了した瞬間にAPIブロックも解除された(直後に読み取り専用リクエストで200
確認)。つまり「同じ日に大量にAPIを叩いた」こと自体が直接の原因ではなく、
その結果としてアカウントが本人確認を要求する状態になり、それが未対応の間は
API全体がブロックされていた、と考えられる。

**次に似た症状(publishだけ失敗・生成は成功)が出た場合の確認順序:**
1. まず`GET /{ig_user_id}?fields=username`のような読み取り専用リクエストで
   ブロック範囲を確認する(投稿系だけかAPI全体か)。
2. Meta開発者ダッシュボードのアラートを確認(今回は何も出ていなかった)。
3. **Instagramアプリ自体を開いて、本人確認/セキュリティチェックを求められて
   いないか確認する。** これが盲点になりやすい。ダッシュボードには出ない。
4. 短時間隔で自動リトライを繰り返さない(ブロックが長引くリスクがあるため、
   数時間おきなど間隔を空けて確認する)。
